...
fn layout_n(
    ...
    let ProposedDimension::Exact(width) = offer.width else {
        let mut total_width: Dimension = 0u16.into();
        let mut max_height: Dimension = 0u16.into();
    ...
    let mut remaining_width =
        width.saturating_sub(spacing * (subview_count.saturating_sub(num_empty_views + 1)) as u16);
