use buoyant::{
    ...
    match_view,
    view::{
        HStack, Rotary, VStack, View, ViewMarker,
        paginate::PageEvent,
        prelude::*,
...
pub fn view(state: &state::State) -> impl View<ColorFormat, state::State> + use<> {
    Paginate::new(
        ...
        Paginate::new(
            GROUP_1,
            |c, evt| {
                c.next_speed_mode = c.operation_state.as_active().map(|o| match evt {
...
fn header(state: &state::State) -> impl View<ColorFormat, state::State> + use<> {
    ...
    let flash_state = state
        ...
        .map(|a| match a.headlight_mode {
            ...
            state::HeadlightMode::On => "☼",
            state::HeadlightMode::Off => "☉",
        })
        .unwrap_or("☉");
    ...
    VStack::new((
        ...
        HStack::new((
            ...
            VStack::new((
                ...
                infocard(123 as i16, "km Range", false),
            ))
            .with_spacing(8),
        ))
        .with_spacing(8)
